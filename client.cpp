...
void ParallelMessage()
{
    ...
    while (!stopThreads.load())
    {
        ...
        // Black magic to sleep for 60'' or until notified
        {
            ...
            cv.wait_for(lock, std::chrono::seconds(60), [&] { return stopThreads.load(); });
        }
    }
}
...
int main(int argc, char *argv[])
{
    ...
    createGraph();
    std::thread drawGraphThread([](int sd, Graph& mapGraph, std::atomic<bool>& stopThreads, std::condition_variable& cv) {
    drawGraph(sd, mapGraph, stopThreads, cv);
}, sd, std::ref(mapGraph), std::ref(stopThreads), std::ref(cv));
